...
//
// ⚠️ 这个数是**维持阈值**，不是自启动阈值。第一版测法给的停转窗口只有 3 秒，
//    风扇靠惯性还在转 —— 所以量到的是"还在转时能不能维持"。自启动阈值必然
//    **高于**这个数，具体多少见下面「自启动阈值单独量」。
//    把维持阈值当自启动阈值用，会以为有余量其实没有。
//
...
//
// 残余风险只有「跑着被外力卡停、需要自己重新起来」。兜底：
//   · 温度一升，档位机推到 25%/40%，那个力度必然带得动 —— 也就是说下面的
//     第 ① 条温度环**顺带就是停转自愈**，这是主要兜底；
...
// 【自启动阈值单独量 —— 待补】
// ============================================================================
// 上面说了，第一版测法量到的是维持阈值。自启动阈值要用足够长的停转窗口
// （board/scripts/fan_minduty_sweep.sh 的 STOP=，默认 20 秒，且要**亲眼确认
// 风扇完全静止**）重新量一遍。
//
// 量出来之后按下面两种情况处理：
//   · 自启动阈值 ≤ 8%：8% 保持不变，兜底里可以再加回"实测能自启动"这一条；
//   · 自启动阈值 > 8%：8% 仍然可用（正常运行用不到自启动，见上），但要在这里
//     如实写下"8% 起不来，卡停后只能靠温度环自愈"，**不许含糊过去**。
// 无论哪种，档位表和两条安全 rail 都不受影响。

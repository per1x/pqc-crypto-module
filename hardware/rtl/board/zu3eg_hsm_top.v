...
// ============================================================================
// 【ML-DSA 的参数集：运行时选，不是综合时选】
// ============================================================================
// 这一份 bitstream 里的 ML-DSA **同时支持 44 / 65 / 87**，软件按 mldsa_axi
// 的 MODE 寄存器 PSET 字段逐次运算选择。所以顶层没有、也不该有"这份
// bitstream 是哪一套"这种参数。
//
// ⚠️ 这里曾经有过一个 MLDSA_PSET 参数和一整套 MLDSA_K/L/ETA/... 透传，
//    那是三个核还只支持编译期单参数集时的形态。留下两条记录：
//
//    ① 更早的一版**连传都没传**，于是 engine 取默认值 —— 板上那份其实是
//       ML-DSA-44，而所有面积/时序估算都按最大的 87 算。"默认值悄悄决定了
//       产品形态"是最不该有的一类含糊：它不写在任何地方，只能从三层默认
//       参数里推出来。
//    ② 运行时可选做通之后，`mldsa_axi` 里那句 `pset == PSET[1:0]` 就变成一道
//       把另外两套全部拒掉的闸门 —— 两侧各自都对，合到一起才错。这种接缝
//       错误不会被任何一侧的用例发现，只有合并后端到端跑才现形。
//
// 如果将来因为面积/时序不得不退回单参数集，退路仍是 87 → 65 → 44，
// **并且必须在交付文档里写清楚上的是哪一套** —— 退是合法的工程取舍，
// 含糊成"ML-DSA 全在硬件"就不是了。
...
`endif
    ...
    // 于是文档必然有一版是错的 —— 这已经发生过一次：RTL 早已切到全 1，
    // 而三份面向使用者的文档还在描述上一版位流。
    //
...
`endif
    ...
    // ============================================================================
    // 这根线原来恒接 0，于是 key_vault 的一拍全清、防火墙的
    ...
    //
    // ⚠️ **参数集不再由综合决定**：engine 运行时支持 44/65/87，软件按 MODE
    //    寄存器的 PSET 字段逐次运算选。顶层这里以前有一整套 MLDSA_K/L/ETA/...
    //    一路传到底，现在全部删掉了 —— 留着会让人以为出 bitstream 前还要选。
    //    MLDSA_PSET 这个参数本身也随之失去意义（见它声明处的说明）。

...
module sm4_core (
    ...
    // 三项都先各自取 mod 256（也就是各写成 8 位）再相加，和在 8 位里回卷 ——
    // 与"算满 9 位再截低 8 位"等值，但位宽从头到尾是对的，不留一根悬空的进位。
    // 16i mod 256 只看 i 的低 4 位，8i、4i 在 i ≤ 31 时本来就装得下 8 位。
    wire [7:0] ck_b0 = {cnt[3:0], 4'd0} + {cnt[4:0], 3'd0}
                                        + {1'b0, cnt[4:0], 2'd0};
    wire [31:0] ck = {ck_b0,
                      ck_b0 + 8'd7,
                      ck_b0 + 8'd14,
    ...
    // rk 只有 32 项，下标是 5 位。cnt 声明成 6 位是为了让 `cnt == 6'd31` 之后
    // 还能再加一拍（S_RND 里数到 32 才切状态），但取轮密钥时它必然在 0..31，
    // 所以正序、倒序两条都用 cnt[4:0]。
    wire [31:0] rk_now = dec_r ? rk[5'd31 - cnt[4:0]] : rk[cnt[4:0]];

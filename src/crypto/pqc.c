...
int pqc_backend_has_hw_keys_kind(unsigned kinds)
{
	...
	if (!be || !kinds) {
		return 0;   /* 不问就不答；空掩码回真会让调用方误以为"都在" */
	}
	/* 生成那一步是两类共用的：没有 keypair_hw，谈不上私钥在硬件里生出来。 */
	if (!be->keypair_hw) {
		return 0;
	}
	/* 每一类各自还要有**用**它的句柄路径。少了这一半，密钥生在硬件里却只能
	 * 把私钥拉回软件才用得上 —— 等于没有。 */
	if ((kinds & (unsigned)PQC_KIND_KEM) && !be->decaps_hw) {
		return 0;
	}
	if ((kinds & (unsigned)PQC_KIND_SIG) && !be->sign_hw) {
		return 0;
	}
	if (kinds & ~((unsigned)PQC_KIND_KEM | (unsigned)PQC_KIND_SIG)) {
		return 0;   /* 问了一个本层不认识的种类，如实说不知道 */
	}
	return 1;
}
...
{
	return pqc_backend_has_hw_keys_kind((unsigned)PQC_KIND_KEM);
}
                         ...
                         uint8_t *sig, size_t *sig_len)
{
	...
	if (!be || !be->sign_hw)
		return PQC_ERR_UNSUPPORTED;
	return be->sign_hw(alg, hw_handle, msg, msg_len, ctx, ctx_len,
	                   rnd, sig, sig_len);
}

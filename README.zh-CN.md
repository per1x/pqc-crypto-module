...
> **状态：研究原型。** 本仓库有两条线，两者的安全边界不同：
>
> - **软件线**（`src/`、`cli/`、`demo/`）：边界是进程地址空间，不是硬件。
>   参见[安全模型与局限](#安全模型与局限)。
> - **FPGA 线**（分支 `zu3eg-fpga-crypto`）：边界是可编程逻辑里一道按 AxPROT
>   门控的 AXI 防火墙，而且**在板上双向证明过** —— 安全世界（EL3）读得到
>   `SECURE_ONLY=1` 的核，普通世界在总线上被拒。见
>   [密码机原型-说明文档.md](docs/密码机原型-说明文档.md)（[PDF](docs/密码机原型-说明文档.pdf)）。
>
> 两条都不是经过认证的模块。请勿用于保护真实资产。
...
本节原先列为"待做"的事，大部分已经在分支 `zu3eg-fpga-crypto` 上做完，并在一块
XCZU3EG 板子上验证过。原条目连同结果一并保留，好让这些说法是**可核对的**而不是
需要相信的：

| 原来写的 | 现在 |
|---|---|
| 用 RTL 实现完整 ML-KEM 数据流——"缺的是时序控制器" | **已完成。** ML-KEM 512/768/1024 的 KeyGen/Encaps/Decaps，**在真硅上**对 NIST ACVP 官方向量逐字节一致（20/20）。参数集由寄存器字段选择，长度在 RTL 里算出来，所以软件报不错 |
| 在目标器件上综合与时序收敛——"脚本已就位但从未运行" | **已完成。** 从 RTL 到 bitstream 的完整流程。35173 LUT（49.85%）、25824 寄存器、15.5 BRAM、140 DSP，**WNS +3.469 ns / WHS +0.011 ns** @75 MHz |
| 把安全边界移入可编程逻辑 | **已完成。** 按 AxPROT 门控的 AXI 防火墙，密钥仓的密钥只走专线出去。板上双向证明：EL3 读得到 `SECURE_ONLY=1` 的核，EL1-NS 被拒（SIGBUS/DECERR），而**同一个**普通世界读得到 `SECURE_ONLY=0` 的核——所以差别来自门控，不是地址不可达 |
| 把密钥派生根移入 eFUSE/BBRAM/PUF | **未做，且在这块板上不打算做。** eFUSE 不可逆而只有一块板；BBRAM 需要 JTAG。见说明文档里的受阻清单 |
| 环形振荡器噪声源 + SP 800-90B 熵评估 | **已完成。** 从板上导出 1,048,576 个**调理前**样本，按 SP 800-90B 非 IID 估计器算得 **H = 0.871234 比特/样本**。这次实测反过来推翻了原先按 H=0.5 假设定的健康检测阈值——APT 那道检测原来根本不会触发——两个阈值都已重算 |
| 测得端到端加速比 | **已测。** ML-KEM-512 为 924 / 1339 / 1018 次每秒（KeyGen/Encaps/Decaps）；768 与 1024 的比例约 1 : 1.5 : 2.1，与 k=2/3/4 的工作量相符 |
...
3. **XMPU/XPPU 的实际配置。** 板上实测：149 个配置寄存器对普通世界**全部拒绝**
   （是总线错误，不是内核限制），所以要配必须有安全世界主控。本次只做了门控证明
   所需的最小安全负载。

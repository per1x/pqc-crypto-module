#!/bin/bash
# 编带 PL_SECREAD SiP 的 BL31，并打成非 golden 槽的 BOOT0004.BIN
#
# ============================================================================
...
# 【为什么敢刷：掉电自动回黄金镜像】
# ============================================================================
# 产物写进 **BOOT0004.BIN**（新文件，不覆盖任何已有槽），靠 CSU_MULTI_BOOT
...
OUT=/home/build/wdt_patch/atf_secread
...
BIF
bootgen -image boot_secread.bif -arch zynqmp -w -o BOOT_SECREAD.BIN 2>&1 | grep -E "ERROR|WARNING" || true
ls -l BOOT_SECREAD.BIN BOOT_OPTEE_FIXQ.BIN
md5sum BOOT_SECREAD.BIN
echo "DONE（只落盘，没刷卡）"

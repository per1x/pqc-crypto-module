...
| `hardware/tb/cocotb/` | — | 195 个测试的对拍台 |
...
```
| CLB LUTs                | 35587 |     0 |     70560 | 50.44 |
| CLB Registers           | 25910 |     0 |    141120 | 18.36 |
...
```
    WNS(ns)   TNS(ns)  TNS Failing Endpoints  TNS Total Endpoints   WHS(ns)   THS(ns)  THS Failing
      3.222     0.000                      0                79041     0.013     0.000            0
...
| 项 | 值 |
|---|---|
| LUT | 35587 / 70560（**50.44%**） |
| 寄存器 | 25910（18.36%） |
| BRAM | 15.5 / 216（7.18%） |
| DSP | 140 / 360（38.89%） |
| 外部管脚 | 1（风扇 `AA11`） |
| **WNS / WHS** | **+3.222 ns / +0.013 ns** @75 MHz |
| 对应 Fmax | 约 **99 MHz**（周期 13.333 ns − WNS 3.222 ns = 10.111 ns 关键路径） |

TRNG + 密钥仓 + AES/SM4/SM3 + ML-KEM 三核 + 金丝雀 + 译码 + 风扇温控 + PS 接口，
占这颗片子**刚好一半**。

> 说明：上表是产品版实现的实测值，bitstream md5
> `7e682c01fbf5357e8e7af9739ea21af0`。
>
> **与上一版的差**：+414 LUT（49.85% → 50.44%）、+86 寄存器，BRAM 与 DSP 不变，
> WNS 从 +3.469 降到 +3.222 ns。这一版新增的是完整地址译码、ML-KEM 的 BRAM
> 擦除机与参数校验、TRNG 的取样 FIFO（见 4.6）。代价是 0.6 个百分点的 LUT 与
> 0.25 ns 的余量，两项都还在收敛区间里，不构成回归。
>
...
$ ./tools/rtl_sim.sh
  ...
  ✓ test_mlkem_axi     mlkem_axi                TESTS=8 PASS=8
  ...
  ✓ test_trng_nodrop   trng_top                 TESTS=1 PASS=1
...
> **状态：复验程序（`board/hsm_audit.c`）与 payload 已就绪并交叉编译通过，
> bitstream 已传到板上（md5 一致），但这一轮上板**尚未跑完** —— 第一次尝试时
> 板子冻住需要断电（原因见 `docs/fpga-进展.md`：复验程序里一笔"预期失败的写"
> 触发 SError，内核 panic；那条检查已删除）。板子恢复后补跑，结果回填到这里。
> **在此之前，上表右侧"能证"三个字描述的是可行性，不是已经取得的结果。**

...
> 说明：Vivado 的布局布线不是逐位可复现的（未固定 seed，`phys_opt` 是启发式）。
> 同一份 RTL 两次实现得到 35104→35173 LUT、WNS 3.317→3.469 ns，属正常波动。
> 上表是最后一次产品版实现的实测值，bitstream md5 `a29b1169527eccf1b7c05f8c83660527`。

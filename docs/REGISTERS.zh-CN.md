...
| `0x0C` | `MODE` | RW | `[1:0]` 操作（0 KeyGen，1 Encaps，2 Decaps），`[3:2]` 参数集（0 = 512，1 = 768，2 = 1024），`[4]` DK_TO_SLOT，`[5]` DK_FROM_SLOT，`[9:6]` SLOT（4 位 —— **16 个槽，不是 4 个**），`[10]` SEED_STAGED |
...
| `0x30` | `KEYSTAT` | R | `[15:0]` 金库里哪些槽装了东西（**16 个**），`[16]` 私钥外泄闩锁，`[17]` 种子闩锁 |
| `0x34` | `KEYPSET` | R | 每槽 2 位的参数集 —— **独立一个寄存器**：16 个槽之后 pset 要 32 位，与有效位挤不进同一个字 |
...
| `0x2C` | `KEYSTAT` | R | `[7:0]` 签名金库里哪些槽装了东西，`[8]` 私钥外泄闩锁，`[9]` **种子闩锁**，`[31:16]` 每槽 2 位的参数集。⚠️ `[8]` 与 `[9]` **互相独立**（两把闩守的方向相反），四种组合都合法，别在软件里假设蕴含关系 |

...
**为什么功能从机是 `SECURE_ONLY=0`。** Linux 跑在普通世界，它经 `/dev/mem`
发出的每一笔事务都带 `AxPROT[1] = 1`。把四个功能从机全设成 1，Linux 就一个
寄存器都读不到——这样的构建能证明"什么都被拒了"，却证明不了算法对不对。
所以功能核是开放的，而在槽位 4 上放一个**只有**这一个参数不同的哨兵实例。
它每被拒一次，就是 AxPROT 门控对着一个真实的非安全主设备实实在在起作用。
产品构建会把四个全设成 1，并由安全世界驱动。
...
`AxPROT[1] == 0`，否则返回 DECERR。有两个细节要紧：

- **被拒的读绝不弹出 FIFO。** 否则一次拿不到数据的非安全读仍然会消耗随机字
  ——那就成了普通世界抽干熵池的手段。
- **是 DECERR，不是 OKAY 加返回零。** 悄悄返回零会让普通世界以为自己读到了
  东西。拒绝必须是可见的。

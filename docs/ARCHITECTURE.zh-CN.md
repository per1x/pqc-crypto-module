...
远端调用时 `libsdfe` 与 `pqchsm_fpgad` 之间那一跳走 **mTLS**（TLS 1.3，双向证书）；
本机那一跳是 0600 的 UNIX socket，不套 TLS（理由见 `docs/SECURITY.zh-CN.md`
的「远程口」一节）。**帧格式两条路逐字节相同** —— 上面这张图里其余部分不受影响。
...
| `0x8006_0000` | `mldsa_axi` | 随形态 | ML-DSA-44/65/87，参数集由 `MODE[3:2]` 决定；片内签名金库 8 槽 |
...
**四个功能从机全是 `SECURE_ONLY=1`。** Linux 跑在普通世界，它经 `/dev/mem`
发出的每一笔事务都带 `AxPROT[1] = 1`。四个全设成 1 之后，Linux 一个密码核
寄存器都读不到。

这个选择决定了 KAT 从哪一侧驱动，而另一种做法值得点名，因为它看着等价、其实不是：
把功能从机开着、另找一个哨兵实例来演示门控，能证明门控有效 —— 但**它证明受保护的
是那个空壳哨兵，不是密码核本身**。所以四个全关，整套 KAT 改由**安全世界**驱动 —— 打过补丁的 BL31 提供一个受限的安全 MMIO SiP，白名单正好覆盖这几个核的
合法寄存器偏移（`boot/atf/patch_atf_secmmio.py`），普通世界的测试程序把每一笔
核访问都经 SMC 转过去。哨兵留在槽位 4 作同参数对照。

⚠️ **白名单少一槽 = 那个核在送检形态下等于不存在。** ML-DSA 落在槽 6，而那张表
一度只到槽 5，于是"位流装好了、核也在，却什么都读不出来"——排查方向会被引向
位流与 RTL，而根因只是一行表项。**加从机就要同时加白名单那一行。**

**两份位流并存，默认起演示那份。**`PQC_DEV_OPEN=1` 出开放构建
（`zu3eg_hsm_dev.bit`，`SECURE_ONLY=0`），不带则出送检构建（`zu3eg_hsm.bit`，
`SECURE_ONLY=1`）。板子默认启动载入**演示形态**：上电即可演示，KAT 与现场排查
不必先换镜像；切送检形态用 `pl_deadman.sh` 运行时载入，网络不丢、不用断电。
**daemon 在两种形态下走的是同一条路**（`/dev/secmmio` → EL3 白名单），所以换
形态不影响服务层——少一条只在某个形态下才验过的路径。

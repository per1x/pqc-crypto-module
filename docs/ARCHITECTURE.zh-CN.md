...
2026-08-18 那节）。**帧格式两条路逐字节相同** —— 上面这张图里其余部分不受影响。
...
现在这一版把缺口补上了。功能核全是 `SECURE_ONLY=1`，整套 KAT 改由**安全世界**
...
位流与 RTL，而根因只是一行表项。2026-08-17 补上槽 6 并重建 BL31 之后，送检形态
下 ML-DSA 才第一次可达。**加从机就要同时加白名单那一行。**

...
| 随机比特生成 | — | 种子、随机数、恢复主密钥 | `RAND_bytes`（OpenSSL） | OpenSSL 的 DRBG。硬件噪声源（`hardware/rtl/trng/`）已做出来并在板上运行——导出 1,048,576 个调理前样本，按 SP 800-90B 非 IID 估计器算得 **H = 0.871234 比特/样本**——但 `src/` 里的**软件**路径仍用 DRBG，只有 FPGA 线消费硬件熵源。 |
...
- 密钥派生根是固定常量，因此没有任何密钥真正与设备绑定。真实设备应从 eFUSE、
  BBRAM 或 PUF 取得该值。
- 模块边界是可编程逻辑：ML-KEM 与对称核、TRNG、密钥仓，以及围住它们的按 AxPROT
  门控的 AXI 防火墙。装进密钥仓的对称密钥不跨越它（实测）。ML-KEM 私钥目前是跨的，
  因为 `KeyGen` 经 AXI 返回 `ek ‖ dk`，这样才能对 ACVP 向量。
- 边界内有一个带 SP 800-90B 健康检测的环形振荡器噪声源，实测最小熵 0.871234
  比特/样本。**主机**软件的随机比特仍来自由操作系统播种的 OpenSSL DRBG。
- 主机侧的密钥处理——keystore 文件及其封装、PKCS#11 层——在边界之外，
  运算期间明文密钥材料存在于该进程地址空间中。

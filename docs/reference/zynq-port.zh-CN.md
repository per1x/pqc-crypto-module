...
**诚实预期**：单蝶形 NTT 核在 100 MHz 下每次变换 3457 周期，约 35 µs（系数存储改成
同步读 BRAM 之后一个蝶形要两拍，见 S3）；A53 上同一
...
`docs/reference/security-policy.zh-CN.md` §13 列出十条阻断送检的差距。移植后的状态：
...
| 10 | 没有完整算法的硬件实现 | ✅ **已全部关闭**：ML-KEM 三参数集、对称核、**以及 ML-DSA 44/65/87 的完整 KeyGen/Sign/Verify** 都已在 PL 里实现并在真硅上对官方向量逐字节一致（登记表 DOC-1：这一格原写"ML-DSA 仍只有算子"，那已经不是事实） |
...
- 侧信道防护。TrustZone 与 XMPU 不提供任何针对功耗、电磁或缓存时序侧信道的防护；
  `docs/reference/constant-time.zh-CN.md` 中的分析针对的是软件时序，在 A53 上需重新审视。

...
>   判据一路到 ACVP 官方向量逐字节。
> * ✅ **2026-08-17 起 ML-DSA 已在硬件上验证**：两种位流形态（演示
>   SECURE_ONLY=0 / 送检 SECURE_ONLY=1）下，三个参数集的 KeyGen/Sign/Verify
>   逐字节对上 ACVP，片内金库与运行时切参数集也过；交付路径（应用 → libsdfe →
>   daemon → /dev/secmmio → EL3 SiP → 核）端到端跑通。证据在 board/logs/。
>   上板同时抓到并修掉了一个仿真看不见的 Verify 缺陷（见 §7 ③）。
> * ⚠️ **一条仍未做，见 §7**：`zeroize` 覆盖不到三个核内部的存储。
...
**② ML-DSA 的上板验证（2026-08-17 已完成，这一条从"未做"划掉）。**
先前卡在 BL31 的 SiP 服务白名单没有 `0x8006_0000` —— 那张表只到槽 5，而
ML-DSA 在槽 6。补一行、重建 BL31、重打 BOOT.BIN 之后，送检形态下 daemon
经 EL3 就摸得到它了。现在两种形态都验过：

| | 演示形态 SECURE_ONLY=0 | 送检形态 SECURE_ONLY=1 |
|---|---|---|
| 普通世界 `/dev/mem` 直连 | 可达，ACVP 自检 32/32 | 三个槽全读回 0（RAZ/WI，不崩板） |
| daemon 经 EL3 白名单 | 三参数集端到端通 | 三参数集端到端通 |

**③ 上板抓到一个仿真看不见的 Verify 缺陷（已修）。**
`ctilde_p == ctilde` 是整 512 位比较，而两个寄存器每次只写低 ctb 字节 ——
高位是上一次运算的残留。上一次判过否，之后每一条 44/65 的合法签名都被拒
（板上 44/65 失败 300/300、87 失败 0/300）。仿真看不见是因为复位把高位清成 0，
而**板上装载之后再也不复位**。详见 mldsa-verify-design 坑表 V8。

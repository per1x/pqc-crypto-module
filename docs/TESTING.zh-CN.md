...
这些数字包含逐字节的软件 AXI 传输，**不是**硬件核的耗时。

> ⚠️ **上面那句"比值与工作量吻合"是错的，2026-08-25 已经证伪。**
>
> 原话说跨参数集的 1 : 1.5 : 2.1 "与 k = 2/3/4 的工作量吻合"。ML-KEM 的算力需求
> 按 **k²** 涨，512/768/1024 应当是 **1 : 2.25 : 4**。而实测的 1 : 1.53 : 2.10
> 正好是**输出字节数**之比（800 / 1184 / 1568 = 1 : 1.48 : 1.96）。
>
> 也就是说这条曲线量的是**管子，不是核** —— 时间随传输字节数走，不随计算量走。
> 一个"符合预期"的比值恰恰掩盖了这一点：数字对得上，但对上的是另一个东西。

## D11：4 字节打包口之后的重测（2026-08-26，同一进程 A/B）

`board/src/iopack_bench.c`，**同一进程、同一份位流、同一批输入**，逐字节跑一遍、
打包跑一遍紧挨着 —— 跨两次运行去比会把时钟、布线、温度混进差值里。
每一对都**逐字节比对两条路的输出**（打包最容易出的错是字节序与错位，那不会报错，
只会让 ek/密文变成另一个完全合法的值）。20 次取平均。

**`/dev/mem` 直连（每字节一笔 AXI）**：

| 参数集 | KeyGen | Encaps | Decaps |
|---|---|---|---|
| ML-KEM-512 | 1.09 → **0.62 ms**（1.76×） | 0.75 → **0.54 ms**（1.37×） | 0.98 → **0.83 ms**（1.18×） |
| ML-KEM-768 | 1.65 → **0.97 ms**（1.70×） | 1.12 → **0.83 ms**（1.34×） | 1.44 → **1.22 ms**（1.18×） |
| ML-KEM-1024 | 2.27 → **1.37 ms**（1.66×） | 1.57 → **1.17 ms**（1.34×） | 1.96 → **1.67 ms**（1.18×） |

**`/dev/secmmio` → EL3（交付通路，每字节一次 ioctl + SMC）**：

| 参数集 | KeyGen | Encaps | Decaps |
|---|---|---|---|
| ML-KEM-512 | 3.07 → **1.11 ms**（2.77×） | 1.96 → **0.85 ms**（2.30×） | 2.65 → **1.25 ms**（2.11×） |
| ML-KEM-768 | 4.56 → **1.69 ms**（2.70×） | 2.84 → **1.26 ms**（2.25×） | 3.85 → **1.83 ms**（2.10×） |
| ML-KEM-1024 | 6.10 → **2.32 ms**（2.63×） | 3.96 → **1.77 ms**（2.23×） | 5.25 → **2.50 ms**（2.10×） |

三件事值得单独指出来：

1. **交付通路本来就比 `/dev/mem` 慢约 2.7 倍**（同样逐字节：4.56 ms vs 1.65 ms）。
   多出来的全是**每字节一次系统调用 + 一次世界切换**。所以打包在这条路上收益更大
   （2.1–2.77× vs 1.18–1.76×）—— 省的不只是总线事务，还有 syscall 与 EL 切换。
2. **收益按操作分层，而且分得很干净**：KeyGen 最大、Decaps 最小。KeyGen 输入只有
   64 字节而输出上千，Decaps 反过来是输入几千字节、输出 32 字节。打包对**读写
   都省**，但两条路各自的固定开销不同，于是比值不一样。这正好再次说明瓶颈在 I/O。
3. **剩下的那一截才是核**。打包之后 Decaps 只快 1.18×（`/dev/mem`），说明它那
   1.67 ms 里已经有相当一部分是真在算了 —— 再打包也压不下去。

⚠️ **即便如此，"更快"仍然不是可以对外说的话。** 这里量到的是同一块板上换一种搬运
方式的差值，不是与任何别的产品的比较。能站住的仍然只有"PQC 在专用硬件通路上跑通
并逐字节对齐 ACVP"。

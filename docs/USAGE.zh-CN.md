...
> ## ⛔ 先读这条：不做任何一次性 / 不可逆的烧写
>
> eFUSE、eMMC 的 RPMB 认证密钥、BBRAM 锁存位、解不开的一次性闩锁 —— 一律不做。
> 仓库里唯一那条路（`board/src/rpmb_tool.c` 的 `provision`）已经在**编译期**
> 关掉，要用得改代码重编（`-DRPMB_ALLOW_PROVISION=1`）。
>
> 这条规则是拿一块板换来的：2026-08-18 那次 `Program Key` **成功了**，工具却
> 读回一份陈的响应帧、报成"失败"，随后密钥文件被删。后来从 SD 的已释放块里
> 找回来了 —— 那是运气，不是流程。经过与判据见
> [SECURITY.zh-CN.md](SECURITY.zh-CN.md) 的 2026-08-18 一节。
>
> 要点不是"下次小心"，而是：**不可逆动作 + 会撒谎的状态读取 = 不可接受。**

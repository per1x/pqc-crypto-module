...
enum {
	...
	/* 8 —— 曾经的 OP_AUTH（TCP 连接的第一帧，载荷是预共享口令）。
	 * **已删除**：远程口改成 mTLS 之后，身份在 TLS 握手里就定下来了，
	 * 应用层再发一条"我是谁"没有增加任何保证。号码留空不复用 ——
	 * 复用会让一个老客户端的第一帧被当成别的操作执行。 */
	OP_AUTH_REMOVED  = 8,
 ...
 * 远程调用密码机（内网演示用）。**帧格式逐字节相同**，区别只有一条：
 *
 *   TCP 连接整个跑在 TLS 1.3 里，**双向证书**。
 *
 * 凭据在板上的 pki/ 目录（0600）：
 *
 *   hsm_ca.crt      设备 CA 的证书 —— 用来验客户端
 *   hsm_device.crt  这台设备自己的证书 —— 客户端用它验板子
 *   hsm_device.key  设备私钥
 *   hsm_acl         可选：允许的客户端 CN 白名单，一行一个
 *
 * **三样（CA/证书/私钥）缺一就根本不监听 TCP** —— fail-closed：宁可远程用
 * 不了，也不要裸奔一个能驱动密码机的端口。
 *
 * ⚠️ 2026-08-18 之前这里是**明文 TCP + 一条静态预共享口令**（OP_AUTH）。
 *    那条路已经整个删掉。它的问题不是"不够强"，是**根本不成立**：口令在
 *    线上是明文的，同网段抓一次包就永久接管；整条会话既不保密也不防篡改；
 *    认证帧本身可以重放。取舍与新设计的完整说明见 service/pqcs_tls.h。
 *
 * 这一层现在挡得住"同网段任何人"与"能抓包/改包的人"。它**仍然不是**
 * 完整的访问控制：没有权限分级、没有按操作的授权，ACL 只到 CN 粒度。
 * 真正的边界仍然在 PL 的防火墙上 —— 那一层与谁在网上说话无关。
 */
#define PQCS_TCP_PORT   9797
/* 凭据的具体路径在 service/pqcs_tls.h（PQCS_CA_FILE 等）——
 * 放那边是因为只有 TLS 那一层用得到它们，而这个头是"线格式"。 */

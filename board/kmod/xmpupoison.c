 ...
 * 【为什么走 PMU，而不是改 BL31】
 * ============================================================================
 * XMPU 的寄存器从 EL1-NS 直写是总线错误，所以本来只能由 BL31 在启动时写。
 * 但 BL31 那条路在本项目当前的 ATF 树上不可靠（重建出来的镜像里 XMPU 那段时灵时
 * 不灵，而且每验一次都要动启动链、动的是唯一一块板）。
 *
 * 于是改走 **PMU 的 PM_MMIO_WRITE 服务**（EEMI API #19）—— 和 pmsec.ko 写
 * CSU_MULTI_BOOT 用的是同一条通路。它由 PMU 固件执行，不需要动 BOOT.BIN。
 *
 * ⚠️ PMU 对 MMIO 写有自己的白名单。**如果 PMU 拒绝，本模块会如实报错并拒绝加载**，
 *    不会假装成功 —— 判据是回读 POISON 寄存器（经 /dev/secmmio 的 EL3 只读窗口，
 *    或直接看本模块打印的返回码）。
 *
 * ============================================================================
 * 【口径：这不是"完整的内存隔离"】
